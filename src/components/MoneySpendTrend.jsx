...
export default function MoneySpendTrend({ data }) {
  const [hoverIdx, setHoverIdx] = useState(null)

  const sorted = useMemo(() => [...data].sort((a, b) => a.cumSpend - b.cumSpend), [data])

  const { slope, intercept } = useMemo(
    () => linearRegression(sorted.map((d, i) => ({ x: i, y: d.meanScore }))),
    [sorted]
  )
  ...
  const fitPoints = sorted
    .map((d, i) => `${scaleX(i)},${scaleY(slope * i + intercept)}`)
    .join(' ')
  ...
  return (
    <div className="chart-wrap money-trend-wrap">
      <svg
        viewBox={`0 0 ${W} ${H}`}
        role="img"
        aria-label="תרשים: הוצאה מצטברת לתלמיד מול ציון PISA ממוצע, ממוין לפי הוצאה, עם קו מגמה"
      >
              ...
              return (
                <g>
                  ...
                  <text x={boxX + 12} y={boxY + 37} fontSize="11" fill="#9ca3af">
                    הוצאה מצטברת: {hovered.cumSpend.toLocaleString()}$
                  </text>
      ...
      <p className="dash-section-note" style={{ marginTop: 4 }}>
        ציר X: מדינות ממוינות לפי הוצאה מצטברת לתלמיד (גילאי 6–15, $ PPP). ציר Y: ציון PISA 2022 ממוצע (מתמטיקה/קריאה/מדעים).
      </p>
    </div>
  )
}
