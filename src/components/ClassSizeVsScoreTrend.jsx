...
export default function ClassSizeVsScoreTrend({ data }) {
  const [hoverIdx, setHoverIdx] = useState(null)

  const sorted = useMemo(
    () =>
      [...data].sort((a, b) => a.classSize - b.classSize || a.country.localeCompare(b.country, 'he')),
    [data]
  )

  const { slope, intercept } = useMemo(
    () => linearRegression(sorted.map((d) => ({ x: d.classSize, y: d.meanScore }))),
    [sorted]
  )
  ...
  const fitPoints = [0, n - 1]
    .map((i) => `${scaleX(i)},${scaleY(slope * sorted[i].classSize + intercept)}`)
    .join(' ')
  ...
  return (
    <div className="chart-wrap money-trend-wrap">
      <svg
        viewBox={`0 0 ${W} ${H}`}
        role="img"
        aria-label="תרשים: גודל כיתה ממוצע מול ציון PISA ממוצע, ממוין לפי גודל כיתה, עם קו מגמה"
      >
              ...
              return (
                <g>
                  ...
                  <text x={boxX + 12} y={boxY + 37} fontSize="11" fill="#9ca3af">
                    גודל כיתה ממוצע: {hovered.classSize}
                  </text>
      ...
      <p className="dash-section-note" style={{ marginTop: 4 }}>
        ציר X: מדינות ממוינות לפי גודל כיתה ממוצע (יסודי), עולה. ציר Y: ציון PISA 2022 ממוצע (מתמטיקה/קריאה/מדעים).
      </p>
    </div>
  )
}
